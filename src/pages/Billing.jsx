  ...
  return (
    <div className="p-6 space-y-8 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">Upgrade to unlock AI diagnosis, revenue tracking, and unlimited sessions</p>
      </div>
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        <button
          ...
          className={`w-12 h-6 rounded-full transition-colors relative ${billing === 'annual' ? 'bg-brand-600' : 'bg-surface-border'}`}
        >
        ...
        <span className={`text-sm ${billing === 'annual' ? 'text-white' : 'text-slate-500'}`}>
          ...
          >
            ...
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-1">
                {plan.id === 'free' && <Sparkles size={15} className="text-slate-400" />}
                {plan.id === 'pro' && <Zap size={15} className="text-brand-400" />}
              ...
              <div className="flex items-end gap-1 mt-3">
                ...
                {plan.price > 0 && <span className="text-slate-500 text-sm mb-1">/mo</span>}
              </div>
              {plan.price > 0 && billing === 'annual' && (
                <p className="text-xs text-slate-500 mt-0.5">billed ${Math.round(plan.price * discount * 12)}/yr</p>
              )}
            ...
            <button
              ...
              className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 ${
                plan.disabled
                  ? 'bg-surface-border text-slate-500 cursor-default'
                  : plan.highlight
                  ? 'bg-brand-600 hover:bg-brand-500 text-white'
                  : 'bg-surface-hover border border-surface-border text-slate-300 hover:border-brand-500 hover:text-white'
              }`}
            >
              {loading === plan.id
      ...
      <div className="bg-surface-card border border-surface-border rounded-xl p-5 space-y-4">
        ...
        {[
          ...
          <div key={q} className="border-b border-surface-border pb-3 last:border-0 last:pb-0">
            <p className="text-sm font-medium text-slate-300 mb-0.5">{q}</p>
